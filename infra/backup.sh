...
backup_assets() {
  # Product images: unencrypted (they're public assets) + incremental sync.
  ...
  rclone sync "minio:${bucket}" "r2:${R2_BUCKET}/images" || log "  (image sync skipped — MinIO unreachable?)"
