 ...
 * for PIN login — or the exact-match query silently fails. Shared between web
 * (server-side PIN verification) and mobile (offline PIN cache lookup) so the
 * two never drift and disagree on what a given phone number normalizes to.
 *
