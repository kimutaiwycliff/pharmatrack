      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 py-4 bg-[var(--pt-surface)] border-b border-[var(--pt-border)] shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 h-9 rounded-lg border border-[var(--pt-border)] text-[var(--pt-text-secondary)] text-sm font-semibold hover:bg-[var(--pt-muted)] transition-colors"
          >
            <FolderTree size={16} />
            Manage categories
          </button>
      ...
      <div className="flex flex-wrap items-center gap-3 px-4 sm:px-6 py-3 bg-[var(--pt-surface)] border-b border-[var(--pt-border)] shrink-0">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full pl-8 pr-8 h-9 text-sm rounded-lg border border-[var(--pt-border)] focus:outline-none focus:ring-2 focus:ring-[var(--pt-green)] bg-[var(--pt-surface)]"
          />
          {rawSearch && (
        ...
        <div className="flex items-center gap-1.5">
          ...
          <select
            ...
            className="h-9 text-sm rounded-lg border border-[var(--pt-border)] px-3 bg-[var(--pt-surface)] focus:outline-none focus:ring-2 focus:ring-[var(--pt-green)] text-[var(--pt-text-secondary)]"
          >
          ...
          <div className="bg-[var(--pt-surface)] rounded-xl border border-[var(--pt-border)] overflow-hidden">
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} className="flex gap-4 px-5 py-4 border-b border-[var(--pt-border)] last:border-b-0 animate-pulse">
                <div className="w-10 h-10 rounded-lg bg-[var(--pt-muted-strong)] shrink-0" />
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {products.map(p => (
                  <tr
                    key={p.id}
                    className={`border-b border-[var(--pt-border)] last:border-b-0 transition-colors hover:bg-[var(--pt-muted)]/60 ${!p.is_active ? "opacity-50" : ""}`}
                  >
                    ...
                    <td className="px-4 py-3.5 text-center">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold border ${
                        p.is_active
                          ...
                          : "bg-[var(--pt-muted)] text-[var(--pt-text-secondary)] border-[var(--pt-border)]"
                      }`}>
                    ...
                    <td className="px-4 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          ...
                          className="px-2.5 h-7 rounded-md text-[12px] font-medium text-[var(--pt-text-secondary)] border border-[var(--pt-border)] hover:bg-[var(--pt-muted)] transition-colors"
                        >
                          Edit
                        </button>
                        <button
          ...
          <div className="flex items-center justify-between mt-4 text-sm text-[var(--pt-text-secondary)]">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 h-8 rounded-lg border border-[var(--pt-border)] disabled:opacity-40 hover:bg-[var(--pt-muted)] transition-colors text-[13px]"
              >
                Previous
              </button>
